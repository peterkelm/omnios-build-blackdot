...
SUMMARY="Provides UEFI BIOS (OVMF) and wrapper for qemu."
DESC="${SUMMARY}"

RUN_DEPENDS_IPS="system/kvm"
BUILD_DEPENDS_IPS=""
...
make_install() {
    ...
    logcmd gtar xjpf ${TMPDIR}/${PROG}.tar.bz2 -C ${DESTDIR}${PREFIX} || \
...
build() {
    ...
    logmsg "--- building"
    logcmd gtar cpvjf ${TMPDIR}/${PROG}.tar.bz2 -C ${SRCDIR}/staging/ . || \
        logerr "------ Failed to pack ${PROG}.tar.bz2"
    make_install
    popd > /dev/null
}
